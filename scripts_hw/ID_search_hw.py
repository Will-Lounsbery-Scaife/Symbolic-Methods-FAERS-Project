...
def FAERS_standard_generate_primaryids_random(path):
    with open("text_files/all_countries.txt", "w") as output:
        for line in path:
            spl = line.split("$")
            if spl[8] in country_list:
                output.write(line)
    filt = open("text_files/all_countries.txt").read().splitlines()
    lset = set([])
    while len(lset) < 5:
...
def add_FAERS_standard_data_to_dictionary(case):
    if case == 1:
        with open("FAERS_standardized/DEMOGRAPHICS.txt", "r") as fp:
            primaryid_gen = FAERS_standard_generate_primaryids(fp)
            primaryid_list = []
